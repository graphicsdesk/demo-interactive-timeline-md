...
let viewBox;
  ...
  var step = article.selectAll('.step');
    ...
    function createCalendar(month, year, container) {
      ...
      // Create SVG container with viewBox for scaling
      ...
      svg.selectAll(".day-box")
          .data(d3.range(daysInMonth))
          .enter().append("rect")
          .attr("x", (d, i) => ((i + firstDay) % 7) * cellSize)
      ...
      events.forEach(event => {
          if (event.date.getUTCMonth() === month && event.date.getUTCFullYear() === year) {
              ...
              const y = Math.floor((day + firstDay - 1) / 7) * cellSize + cellSize + 1.5 * cellSize;
  ...
  /*
  // Create calendars for the specified months
